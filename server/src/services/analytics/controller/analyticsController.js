...
export class AnalyticsController{
    constructor({analyticsService,authService,clientRepository}){
         if(!analyticsService || !authService || ! clientRepository){
            throw new Error(`AnalyticController requires analyticService, authService and clientRepository`)
         }
    ...
    async getStats(req,res,next){
         try {
            ...
            res.status(200).json(
                ResponseFormatter.success(stats,"Overall analytics stats retrieved successfully",200)   
            )
         } catch (error) {
            next(error);
         }
